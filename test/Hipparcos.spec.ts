...
describe('Hipparcos', function() {
    ...
    describe('properMotion', function() {
        it ('Alpha Centauri', function() {
            ...
            const starData2020 = Hipparcos.properMotion(starData, 2458849.50000 + 0.5);
    ...
    describe('hipparcosGet', function() {
        it('SingleMatch', function() {
            const epochJ2000 = 2451544.5000000;
            const timeCorr = new TimeCorrelation();
            const timeStamp = timeCorr.computeTimeStamp(epochJ2000, TimeConvention.TIME_TDB, false);

            const vega = Hipparcos.hipparcosGet("3 Alpha Lyrae (Vega)", timeStamp);
        ...
        it('ProperMotion', function() {
            ...
            const JT_1980 = JulianTime.timeJulianYmdhms(1980, 1, 1, 0, 0, 0);
            const JT_2000 = JulianTime.timeJulianYmdhms(2000, 1, 1, 0, 0, 0);
            const JT_2020 = JulianTime.timeJulianYmdhms(2020, 1, 1, 0, 0, 0);

            const epochJ1991_25 = 2448349.0625;
            const timeCorr = new TimeCorrelation();
            const timeStamp_1980 = timeCorr.computeTimeStamp(JT_1980, TimeConvention.TIME_TDB, false);
            const timeStamp_2000 = timeCorr.computeTimeStamp(JT_2000, TimeConvention.TIME_TDB, false);
            const timeStamp_2020 = timeCorr.computeTimeStamp(JT_2020, TimeConvention.TIME_TDB, false);

            const vega_1980 = Hipparcos.hipparcosGet("3 Alpha Lyrae (Vega)", timeStamp_1980);
            const vega_2000 = Hipparcos.hipparcosGet("3 Alpha Lyrae (Vega)", timeStamp_2000);
            const vega_2020 = Hipparcos.hipparcosGet("3 Alpha Lyrae (Vega)", timeStamp_2020);
