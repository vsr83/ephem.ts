...
export interface NutationData {
    // The timestamp for which the nutation data has been computed.
    timeStamp : TimeStamp, 
...
export class Nutation {
     ...
     *      Timestamp.
     * @returns {NutationData} The nutation parameters eps, deps and dpsi (degrees).
     */
    static iau1980(timeStamp : TimeStamp) : NutationData
    {
        ...
        const N = iau1980NutationData.length;
        let dpsi = 0.0;
        let deps = 0.0;
        // Note j in (A.25) runs from 1 to N but 0 to N-1 below:
        for (let j = 0; j < N; j++)
        {
            const data = iau1980NutationData[j];
        ...
        return {timeStamp : timeStamp, eps : eps, deps : deps % 360.0, dpsi : dpsi % 360.0};
    }
};