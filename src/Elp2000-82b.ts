import elp2000Data from '../data/ELP2000-82b.json';
...
const planetaryPolynomialCoefficients : number[][] = [
...
interface DataFormatNonPlanetary 
{
    i1 : number;
    i2 : number;
    i3 : number;
 ...
 * @param {number} n 
 *      Polynomial degree.
 * @returns {ElpArguments} Object with values W1, W2, W3, T and OBP.
 */
function evaluateArgumentPolynomials(t : number, n : number) : ElpArguments
{
 ...
 * 
 * @param {ElpArguments} elpArguments 
 *      The arguments computed by above routine.
 * @returns {DelArguments} Object with values D, LP, L and F.
 */
function evaluateDelaunayArguments(elpArguments : ElpArguments) : DelArguments 
{
    let delArguments = {D : 0, LP : 0, L : 0, F : 0};
 ...
 * of the main problem.
 * 
 * @param {DelArguments} delArgs 
 *      An object with the Delaunay arguments computed for the given point in time.
 * @param {DataFormatMainFig[]} data 
 *      The ELP01/ELP02 data as an array of objects.
 * @returns {number} The sum.
 */
function computeMainFigureSin(delArgs : DelArguments, data : DataFormatMainFig[])
    : number
{
    let sum = 0.0;
 ...
 * @returns {number} The sum.
 */
function computeMainFigureCos(delArgs : DelArguments, data : DataFormatMainFig[])
    : number
{
    let sum = 0.0;
    for (let i = 0; i < data.length; i++)
 ...
 * @param {DelArguments} delArgs 
 *      An object with the Delaunay arguments computed for the given point in time.
 * @param {DataFormatNonPlanetary} data 
 *      The ELPXX data as an array of objects.
 * @returns {number} The sum.
 */
function computeNonPlanetary(precession : number, delArgs : DelArguments, 
    data : DataFormatNonPlanetary[]) : number
{
    let sum = 0.0;
    for (let i = 0; i < data.length; i++)
    {
 ...
 * 
 * @param {number[]} planetaryArgs
 *      Planetary arguments.
 * @param {DelArguments} delArgs 
 *      An object with the Delaunay arguments computed for the given point in time.
 * @param {DataFormatPlanetary} data 
 *      The ELPXX data as an array of objects.
 * @returns {number} The sum.
 */
function computePlanetary1(planetaryArgs : number[], delArgs : DelArguments, 
    data : DataFormatPlanetary[]) : number
{
    let sum = 0.0;
    for (let i = 0; i < data.length; i++)
    {
 ...
 * @returns {number} The sum.
 */
function computePlanetary2(planetaryArgs : number[], delArgs : DelArguments, 
    data : DataFormatPlanetary[]) : number
{
    let sum = 0.0;
    for (let i = 0; i < data.length; i++)
...
/**
 * Class with a static method for the computation of the Moon position.
 */
export class Elp2000
{
    /**
     ...
     */
    static osvEclGeo(timeStamp : TimeStamp) : OsvFrame
    {
        // Julian centuries after epoch.
        const T = (timeStamp.JTtdb - 2451545.0)/36525.0;
        ...
        return {
            ...
            position : MathUtils.vecMul([xJ2000, yJ2000, zJ2000], 1000),
            velocity : [0, 0, 0],
            timeStamp : timeStamp
        }
    }
